...
st.set_page_config(page_title="VRC / HRV RRi Analyzer Pro v11.2", layout="wide")
...
PARAM_GROUPS = {
    ...
    "Frecuencia": ["VLF", "LF", "HF", "TOTAL", "LF_HF", "VLF_LS", "LF_LS", "HF_LS", "TOTAL_LS", "LF_HF_LS", "VLF_AR", "LF_AR", "HF_AR", "TOTAL_AR", "LF_HF_AR", "LF_WAV", "HF_WAV", "LF_HF_WAV"],
    "Complejidad": ["DFA_alpha1", "DFA_alpha2", "D2", "ApEn", "SampEn", "Hurst", "KatzFD", "PetrosianFD", "DispEn"],
...
def wavelet_band_metrics(rr):
    """
    Aproximación tiempo-frecuencia con STFT como sustituto ligero de wavelet.
    Devuelve potencia media LF/HF variable en el tiempo.
    """
    try:
        ti, xi = interpolate_rr(rr, fs=FS_INTERP, apply_lambda=True, lam=LAMBDA_DEFAULT)
        x = xi * 1000.0
        x = x - np.mean(x)
        if len(x) < 64:
            return {"LF_WAV": np.nan, "HF_WAV": np.nan, "LF_HF_WAV": np.nan}

        nperseg = min(256, len(x))
        f, tt, Zxx = signal.stft(x, fs=FS_INTERP, window="hann", nperseg=nperseg, noverlap=nperseg//2, boundary=None)
        p = np.abs(Zxx) ** 2
        ...
        def band_mean(lo, hi):
            ...
            if not np.any(mask):
                return np.nan
            band = np.trapezoid(p[mask, :], f[mask], axis=0)
            return float(np.nanmean(band))

        lf = band_mean(0.04, 0.15)
        hf = band_mean(0.15, 0.40)
        return {"LF_WAV": lf, "HF_WAV": hf, "LF_HF_WAV": lf / hf if pd.notna(hf) and hf > 0 else np.nan}
    except Exception:
        return {"LF_WAV": np.nan, "HF_WAV": np.nan, "LF_HF_WAV": np.nan}
...
st.title("VRC / HRV RRi Analyzer Pro v11.2")
